...
void MGI_BoatPlanner::initialize()
{
...
// void MGI_BoatPlanner::targetpose_2d_cb(const geometry_msgs::PoseStamped::ConstPtr& msg)
// {
//   if (move_group_) {
//     move_group_->stop();
//   }
...
// void MGI_BoatPlanner::targetpoint_3d_cb(const geometry_msgs::PointStamped::ConstPtr& msg)
// {
//   if (move_group_) {
//     move_group_->stop();
//   }
...
void MGI_BoatPlanner::hardcoded_3d_cb() {
  ROS_INFO("Hardcoded Movement Works");
  if (move_group_) {
    move_group_->stop();
  }
  else {
  ...
  // geometry_msgs::PointStamped target_point_received = 0;
  // // Ensure that end-effector reference is in world frame 
  // tf::Stamped<tf::Point> t_EE_ref_W;
  // tf::pointStampedMsgToTF(target_point_received, t_EE_ref_W);
  // T_EE_ref_W_ = tf::Stamped<tf::Pose>(tf::Pose(/*tf::Quaternion(0,0,0,1)*/T_EE_W_.getRotation(), static_cast<tf::Point>(t_EE_ref_W)), t_EE_ref_W.stamp_, t_EE_ref_W.frame_id_);

  T_EE_ref_W_ = tf::Stamped<tf::Pose>(tf::Pose(tf::Quaternion(0,0,0,1), tf::Point(1,0,0.5)), ros::Time::now(), world_link_);
  if (T_EE_ref_W_.frame_id_ != world_link_) {
    ROS_WARN_STREAM("move_group_interface: Goal in frame [" << T_EE_ref_W_.frame_id_ << "], have to transform into [" << world_link_ << "]"); 
    //TODO
    throw std::runtime_error("move_group_interface: Wrong frame in received msg");
...
void MGI_BoatPlanner::arm_reach() {
